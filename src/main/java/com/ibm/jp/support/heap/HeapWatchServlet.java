...
public class HeapWatchServlet extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if (str != null) {
			...
			if (str.equals("Reset") && watcher != null)
				watcher.init();
			if (str.equals("Stop")) {
				if (watcher != null && watcher.isAlive())
					watcher.stopWatcher();
			...
			if (str.equals("Update") && watcher != null && watcher.isAlive()) {
				int i;
				try {
			...
			if (str.equals("Load on Heap")) {
				String s = req.getParameter("size");
				int size = (s != null)? Integer.parseInt(s) : 128000;
System.out.println(size);
				storage.add(new byte[size]);
			}
			if (str.equals("Unload All")) {
				storage.clear();
			}
			if (str.equals("Gerbage Collection")) {
				System.gc();
			}
		}
		...
		if (watcher != null) {
			...
			out.println("<INPUT type=\"SUBMIT\" value=\"Show Statstic\" name=\"Statstic\">");
			out.println("</FORM>");
			out.println("<HR>");
		}
		out.println("<FORM action=\"" + req.getRequestURI() + "\" method=\"GET\">");
	...
	private static final Vector<byte[]> storage = new Vector<byte[]>();
